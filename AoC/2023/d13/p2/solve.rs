...
fn is_vertica_reflection(map: &Map, col1: usize, col2: usize) -> i64 {
    let mut error_counts = 0;
    for r in map.iter() {
        if r[col1] != r[col2] { error_counts += 1; }
        if error_counts > 1 { return error_counts; }
    }
...
fn check_vertical_reflection(map: &Map, col1: usize, col2: usize) -> bool {
    ...
    let mut error_counts = is_vertica_reflection(map, col1, col2);

    for idx in 1..=steps {
        let c1 = col1 - idx;
        let c2 = col2 + idx;

        error_counts += is_vertica_reflection(map, c1, c2);
        if error_counts > 1 { return false; }
    }
    error_counts == 1
}
...
fn get_vertical_reflection(map: &Map) -> i64 {
    ...
    for idx in 1..max_col {
        if is_vertica_reflection(map, idx - 1, idx) > 1 { continue; }
...
fn is_hortizontal_reflection(map: &Map, row1: usize, row2: usize) -> i64 {
    let mut error_counts = 0;
...
fn check_horizontal_reflection(map: &Map, row1: usize, row2: usize) -> bool {
    ...
    let mut error_counts = is_hortizontal_reflection(map, row1, row2);

    for idx in 1..=steps {
        let r1 = row1 - idx;
        let r2 = row2 + idx;

        error_counts += is_hortizontal_reflection(map, r1, r2);
        if error_counts > 1 { return false; }
    }
    error_counts == 1
}
...
fn get_horizontal_reflection(map: &Map) -> i64 {
    ...
    for idx in 1..max_row {
        if is_hortizontal_reflection(map, idx - 1, idx) > 1 { continue; }
